...
def preprocess(inputs):
    ...
    atom_fea = np.vstack(
        [ari.get_atom_fea(crystal[j].specie.number) for j in range(len(crystal))]
    )
...
parser.add_argument(
    "--r", type=float, default=8, help="The cutoff radius for searching neighbors"
)
parser.add_argument(
    "--dmin",
    type=float,
...
parser.add_argument(
    "--idprop", type=str, default="id_prop.csv", help="id-prop.csv file"
)
