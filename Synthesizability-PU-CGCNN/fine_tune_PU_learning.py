...
from cgcnn.data_PU_learning import (
    collate_pool,
...
parser.add_argument(
    "--bag", default=50, type=int, metavar="N", help="Bagging size of PU training"
)
parser.add_argument(
    "--split",
    default="./saved_splits",
    type=str,
    metavar="N",
...
parser.add_argument(
    "--restart", default=0, type=int, metavar="N", help="Set restart point of bagging #"
)
...
def main():
    ...
    split_bagging(
        os.path.join(args.cifs, "id_prop.csv"), args.restart, args.bag, args.split
    ),
    ...
    for bagging in range(args.restart, args.restart + args.bag):
        ...
        dataset_train = preload(
            preload_folder=graph_dir,
            id_prop_file=os.path.join(
                args.split, "id_prop_bag_" + str(bagging + 1) + "_train.csv"
            ),
        )
        dataset_valid = preload(
            preload_folder=graph_dir,
            id_prop_file=os.path.join(
                args.split, "id_prop_bag_" + str(bagging + 1) + "_valid.csv"
            ),
        )
        dataset_test = preload(
            preload_folder=graph_dir,
            id_prop_file=os.path.join(
                args.split, "id_prop_bag_" + str(bagging + 1) + "_test-unlabeled.csv"
            ),
        )
        ...
        print(
            "--------------------------------------------------Data loaded in bagging #%d, Time: %f"
            % (bagging + 1, preload_time)
        )
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            mae_error, recall_value = validate(
                val_loader, model, criterion, normalizer, bagging + 1
            )
        ...
        print(
            "In bagging %d, Best epoch = %d, Best AUC = %f, Best recall = %f"
            % (bagging + 1, best_epoch, best_mae_error, best_recall)
        )
        print("---------Evaluate Model on Test-unlabeled Set---------------")
        best_checkpoint = torch.load("checkpoint_bag_" + str(bagging + 1) + ".pth.tar")
        model.load_state_dict(best_checkpoint["state_dict"])
        _, bestrecall_bagging = validate(
            test_loader, model, criterion, normalizer, bagging + 1, test=True
        )
